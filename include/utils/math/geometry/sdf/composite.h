...
#include "mixed.h"
#include "../interactions/ab_mixed.h"
...
namespace utils::math::geometry::sdf
	...
	struct composite
		...
		utils_gpu_available constexpr geometry::sdf::closest_point_with_signed_distance closest_with_signed_distance() const noexcept
			...
			geometry::sdf::closest_point_with_signed_distance result;
			for (const auto& shape : shapes)
				{
				result.set_to_closest(shape.sdf(point).closest_with_signed_distance());
				}
			return result;
			}
		...
		utils_gpu_available constexpr geometry::sdf::side side() const noexcept
			...
			//for (const auto& shape : shapes)
			//	{
			//	const auto shape_side{shape.sdf(point).side()};
			//	if (!shape_side.is_exactly_coincident())
			//		{
			//		side *= shape_side.sign();
			//		}
			//	}
			//return {side};
