...
namespace utils::math::geometry::shape
	...
	utils_gpu_available constexpr auto cast_storage(const concepts::mixed auto& shape) noexcept
		{
		const typename cast_storage_type<std::remove_cvref_t<decltype(shape)>, desired_storage_type>::type ret{shape};
		return ret;
		}
	template <storage::type desired_storage_type>
	utils_gpu_available constexpr auto cast_storage(concepts::mixed auto& shape) noexcept
		{
