...
namespace utils::math::geometry::interactions
	...
	template <shape::concepts::shape T> utils_gpu_available constexpr auto scale    (const T& shape, const float                    & scaling    ) noexcept { typename T::nonref_self_t ret{shape}; return interactions::scale_self    (ret, scaling    ); }
	template <shape::concepts::shape T> utils_gpu_available constexpr auto rotate   (const T& shape, const angle::base<float, 360.f>& rotation   ) noexcept { typename T::nonref_self_t ret{shape}; return interactions::rotate_self   (ret, rotation   ); }
	template <shape::concepts::shape T> utils_gpu_available constexpr auto translate(const T& shape, const vec2f                    & translation) noexcept { typename T::nonref_self_t ret{shape}; return interactions::translate_self(ret, translation); }
