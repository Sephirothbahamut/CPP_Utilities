...
namespace utils::math::geometry::interactions
	...
	utils_gpu_available constexpr typename auto& transform_self(shape::concepts::has_vertices auto& shape, const transform2                 & transform  ) noexcept 
		{
		for(auto& vertex : shape.vertices) 
			{
			transform_self(vertex, transform  );
			} 
		return shape; 
		}
	}